...
-   The row is a leadhead or division starting row; **and**
-   The row matches any of the specified coursehead masks.

If a coursehead satisfied these conditions but is not actually rung during the course to which it belongs, it will be included in the layout (in parentheses).

### Default coursehead masks
All compositions must have at least one coursehead mask. If you do not specify one, Complib will automatically infer a coursehead mask based on the composition's specified [stage](#stage). The default coursehead mask will generally be of the form `x...xN`, where *N* is the largest numbered bell at the composition's specified stage.

### Missing courseheads
If there are courses in the composition which do not contain rows matching any coursehead mask(s), then:
...
## Start row number
Allows you to specify which row of the first [specified method](adding_compositions_tabs_methods.md) the composition starts from. Most methods are rung starting from row 0.

If no start row number is specified, the field defaults to a value of 0. 

If the start row number is anything other than 0, the starting row will be noted underneath the calling in the [composition layout](overview.md/#layout). The exceptions are compositions which [begin at the treble's backstroke snap](#starting-at-the-backstroke-snap), and compositions of [Stedman](#starting-rows-in-compositions-of-stedman).
